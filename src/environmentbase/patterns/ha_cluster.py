...
class HaCluster(Template):
    ...
    Contains an ELB, Autoscaling Group, Security Groups, and optional internal DNS name
    """
    ...
    def __init__(self, 
        ...
        # This is the type of the ELB (Internal or External)
        # TODO: Implement this toggle in template.add_elb
    ...
    def build_hook(self):
        ...
        # This creates security groups for the ASG, the ELB, and connects them together
        security_groups = self.add_security_groups()

        # This creates the ELB, opens the specified ports, and attachs the security group and logging bucket
    ...
    def add_security_groups(self):
        ...
        for elb_port in self.elb_ports:
            elb_sg_ingress_rules.append(ec2.SecurityGroupRule(FromPort=elb_port, ToPort=elb_port, IpProtocol='tcp', CidrIp=self.vpc_cidr))
